...
class Tensor:
    def __init__(
        ...
        match view:
            case View(): self._view = view
            case Tuple(): self._view = View.create(view)
            case None:
    ...
    def _unary_op(self, op: UnaryOps) -> Tensor:
        ...
        idxs = View.unary_indexing(self._view, out._view)
    ...
    def _binary_op(self, other: Union[Tensor, float, int], reverse: bool, op: BinaryOps) -> Tensor:
        ...
        idxs = View.binary_indexing(self._view, other._view, out._view)
    ...
    def _movement_op(self, view: View, op: MovementOps) -> Tensor:
        match op:
            case MovementOps.RESHAPE:
                ...
                out._backward = lambda: self.grad.__iadd__(out._grad.reshape(self.shape))
    ...
    @property
    def data(self) -> np.ndarray: return np.array(self._data.to_vec()).reshape(self._view.shape)
    @property
    def grad(self) -> np.ndarray: return self._grad.data()
    @property
    def shape(self) -> Tuple: return self._view.shape
    @property
