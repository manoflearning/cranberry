...
class Tensor:
    def __init__(self,
                 ...
                 op: Optional[Op] = None):  # TODO: dtype, device
        # self._data
        ...
        self._grad: np.ndarray = grad if grad is not None else np.zeros_like(
            self._data)
        ...
        assert self._shape == self._data.shape, f"shape {self._shape} must match data shape {self._data.shape}"
        assert self._shape == self._grad.shape, f"shape {self._shape} must match grad shape {self._grad.shape}"
    ...
    def _unary_op(self, op: UnaryOps) -> Tensor:
        ...
        if op == UnaryOps.NEG:
            ...
            def backward(): self._grad += 0.5 * out._grad / out._data
            out._backward = backward
        elif op == UnaryOps.RELU:
            out._data = self._data * (self._data > 0)
            def backward(): self._grad += out._grad * (self._data > 0)
            out._backward = backward
        elif op == UnaryOps.EXP:
            out._data = np.exp(self._data)
            def backward(): self._grad += out._grad * out._data
            out._backward = backward
        elif op == UnaryOps.LOG:
            out._data = np.log(self._data)
            def backward(): self._grad += out._grad / self._data
            out._backward = backward
        else:
    ...
    def tanh(self) -> Tensor: return (self.exp() -
                                      self.neg().exp()) / (self.exp() + self.neg().exp())
    # TODO: gelu sanity check

    def gelu(self) -> Tensor: 0.5 * self * \
    ...
    def _binary_op(self, other: Union[Tensor, int, float], reverse: bool, op: BinaryOps) -> Tensor:
        ...
        out = Tensor._dummy(
            shape=self._shape, requires_grad=self.requires_grad or other.requires_grad, prev=(self, other), op=op)
        if op == BinaryOps.ADD:
    ...
    def add(self, other: Union[Tensor, int, float],
            reverse: bool = False) -> Tensor: return self._binary_op(other, reverse, BinaryOps.ADD)

    def sub(self, other: Union[Tensor, int, float],
            reverse: bool = False) -> Tensor: return self._binary_op(other, reverse, BinaryOps.SUB)

    def mul(self, other: Union[Tensor, int, float],
            reverse: bool = False) -> Tensor: return self._binary_op(other, reverse, BinaryOps.MUL)
    def div(self, other: Union[Tensor, int, float],
            reverse: bool = False) -> Tensor: return self._binary_op(other, reverse, BinaryOps.DIV)
        ...
        if op == ReduceOps.SUM:
            ...
            def backward():
                if dim is None or keepdim:
                    ...
                    o_new_shape = tuple(
                        1 if i == dim else s for i, s in enumerate(self.shape))
                    self._grad += (self._data == out._data.reshape(o_new_shape)
    ...
    def _movement_op(self, *args, op: MovementOps) -> Tensor:
        ...
        if op == MovementOps.RESHAPE:
            ...
            def backward():
                ...
                self._grad += out._grad.sum(axis=axis,
                                            keepdims=True).reshape(self.shape)
            out._backward = backward
    ...
    def reshape(self, *shape: int) -> Tensor:
        ...
        assert all(s > 0 or s == -
                   1 for s in shape), "shape dimensions must be positive or -1"

        if shape.count(-1) == 1:
            assert prod(self._shape) % -prod(
                shape) == 0, f"cannot reshape tensor of size {prod(self._shape)} into shape {shape}"
            shape = tuple(s if s != -1 else prod(self._shape) // -
                          prod(shape) for s in shape)
        assert prod(shape) == prod(
            self._shape), f"cannot reshape tensor of size {prod(self._shape)} into shape {shape}"
    ...
    def expand(self, *shape: int) -> Tensor:
        assert len(shape) >= len(
            self.shape), f"the expanded shape {shape} must have at least as many dimensions as the original shape {self.shape}"
        assert all(s == 1 or s == e for s, e in zip(
            self.shape, shape[-len(self.shape):])), "the expanded shape must be compatible with the original shape"
        return self._movement_op(shape, op=MovementOps.EXPAND)

    def permute(self, *dims: int) -> Tensor:
        assert set(dims) == set(range(len(self.shape))
                                ), "invalid permutation {dims}"
        return self._movement_op(tuple(self.shape[dims[i]] for i in range(len(dims))), dims, op=MovementOps.PERMUTE)

    def flatten(self, start_dim: int = 0, end_dim: int = -1) -> Tensor:
        if end_dim == -1:
            end_dim = len(self.shape)
        assert 0 <= start_dim < end_dim <= len(
            self.shape), "invalid start_dim or end_dim"
    ...
    def matmul_2d(self, other: Tensor) -> Tensor:
        assert len(self.shape) == 2 and len(
            other.shape) == 2, "matmul_2d only supports 2D tensors, but got shapes {self.shape} and {other.shape}"
        assert self.shape[1] == other.shape[
            0], f"matmul_2d shape mismatch: {self.shape} and {other.shape}"
    ...
    def matmul(self, other: Tensor) -> Tensor:
        ...
        if len(self.shape) == 1 and len(other.shape) == 1:
            assert self.shape[0] == other.shape[
                0], f"matmul shape mismatch: {self.shape} and {other.shape}"
        ...
        # the non-matrix (i.e. batch) dimensions are broadcasted (and thus must be broadcastable)
            ...
            raise RuntimeError(
                f"Invalid matmul shapes {self.shape} and {other.shape}")

    def dot(self, other: Tensor) -> Tensor: self.matmul(other)
    ...
    def layer_norm(self, weight: Tensor,
                   bias: Optional[Tensor], eps: float = 1e-5) -> Tensor: NotImplementedError

    def dropout(self, p: float) -> Tensor: NotImplementedError

    def sparse_categorical_crossentropy(self, Y: Tensor) -> Tensor:
        assert len(self.shape) == 2 and len(
            Y.shape) == 1, f"sparse_categorical_crossentropy only supports 2D tensor and 1D tensor, but got shapes {self.shape} and {Y.shape}"
        assert self.shape[0] == Y.shape[
            0], f"sparse_categorical_crossentropy shape mismatch: {self.shape} and {Y.shape}"
        ...
        Y_onehot_data[np.arange(len(Y._data)),
                      (Y._data + 1e-5).astype(np.int32)] = 1
    ...
    def uniform(*shape: int, low=0.0, high=1.0) -> Tensor:
        np.random.seed(Tensor._nxt_seed())
        return Tensor(data=np.random.uniform(low, high, prod(shape)).reshape(shape), shape=shape, requires_grad=True, prev=None, op=None)

    @staticmethod
    def kaiming_uniform(*shape: int, a: float = 0.01) -> Tensor:
        bound = math.sqrt(3.0) * math.sqrt(2.0 / (1 + a ** 2)
                                           ) / math.sqrt(prod(shape[1:]))
    ...
    def detach(self) -> Tensor: return Tensor(data=self._data,
                                              shape=self._shape, requires_grad=False, prev=None, op=None)
