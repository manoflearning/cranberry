...
def strides_for_shape(shape: Tuple[int]) -> Tuple[int]:
    if not shape:
        return ()
    strides = tuple(itertools.accumulate(
        reversed(shape[1:]), operator.mul, initial=1))
...
class View:
    ...
    def create(shape: Tuple[int], strides: Optional[Tuple[int]] = None):
        strides = canonicalize_strides(
            shape, strides) if strides else strides_for_shape(shape)
    ...
    def reshape(self, new_shape: Tuple[int]) -> Optional[View]:
        ...
        assert all(
            s >= 0 for s in new_shape), f"shape can't contain negative numbers {new_shape}"
        assert prod(self.shape) == prod(
            new_shape), f"cannot reshape {self.shape} to {new_shape}"
        if 0 in self.shape:
        ...
        if self.contiguous:
            ...
            assert False, "non-contiguous reshape not implemented"

    @functools.lru_cache(maxsize=None)
    def expand(self, new_shape: Tuple[int]) -> View:
        assert len(new_shape) == len(
            self.shape), f"cannot expand {self.shape} to {new_shape}"
        assert all((ns == s or ns == 1) or (s == 1 and st == 0) for ns, s, st in zip(
            new_shape, self.shape, self.strides)), f"invalid expansion {new_shape} from {self.shape=} {self.strides=}"
        return View.create(new_shape, self.strides)

    @functools.lru_cache(maxsize=None)
    def permute(self, dims: Tuple[int]) -> View:
        assert set(dims) == set(range(len(dims))
                                ), f"invalid permutation {dims}"
    ...
    def indices(self) -> Tuple[int, int]:
        ...
        def _indices(shape: Tuple[int], strides: Tuple[int], start: int):
            ...
            for i in range(shape[0]):
                out += _indices(shape[1:], strides[1:],
                                start + i % strides[0] * strides[0])
            return out
