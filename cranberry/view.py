...
class View:
    ...
    def reshape(self, new_shape: Tuple[int]) -> Optional[View]:
        ...
        if 0 in self.shape:
            assert (
                0 in new_shape
            ), f"cannot reshape 0 size shape {self.shape} to {new_shape}"
    ...
    def expand(self, new_shape: Tuple[int]) -> View:
        assert len(new_shape) == len(
            self.shape
        ), f"cannot expand {self.shape} to {new_shape}"
        assert all(
            (ns == s or ns == 1) or (s == 1 and st == 0)
            for ns, s, st in zip(new_shape, self.shape, self.strides)
        ), f"invalid expansion {new_shape} from {self.shape=} {self.strides=}"
    ...
    def permute(self, dims: Tuple[int]) -> View:
        ...
        return View.create(
            tuple(self.shape[d] for d in dims), tuple(self.strides[d] for d in dims)
        )
    ...
    def indices(self) -> Tuple[int, int]:
        ...
        def _indices(shape: Tuple[int], strides: Tuple[int], start: int):
            ...
            for i in range(shape[0]):
                out += _indices(
                    shape[1:], strides[1:], start + i % strides[0] * strides[0]
                )
            return out
