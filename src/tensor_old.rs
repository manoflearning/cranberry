...
impl Tensor_ {
    ...
    pub fn broadcasted(&self, other: Tensor_) -> (Tensor_, Tensor_) {
        let self_read = self.0.read().unwrap();
        let other_read = other.0.read().unwrap();

        let self_dim = self_read.data.raw_dim();
        let other_dim = other_read.data.raw_dim();

        if self_dim != other_dim {
            let self_data = self_read.data.clone();
            let other_data = other_read.data.clone();
            
            let shape_delta = self_dim.ndim() - other_dim.ndim();
...
impl Tensor_ {
    pub fn pow(&self, exp: f32) -> Tensor_ {
    ...
    pub fn relu(&self) -> Tensor_ {
        let data = self.0.read().unwrap().data.clone();
        let out_data = data.mapv(|x| x.max(0.0));
        let out = Tensor_::new(out_data);
        {
            let mut out_inner = out.0.write().unwrap();
            out_inner.children.push((self.clone(), data.mapv(|x| if x > 0.0 { 1.0 } else { 0.0 })));
        }
        out
    }
}
...
impl Tensor_ {
    ...
    pub fn matmul(&self, other: &Tensor_) -> Self {
        ...
        // TODO: implement the correct backward
        let out_data = ArrayD::from_shape_vec(IxDyn(&[dim_0, dim_2]), out_data).unwrap();
        let out = Tensor_::new(out_data);
        {
            let mut out_inner = out.0.write().unwrap();
            out_inner.children.push((self.clone(), self_read.data.clone()));
            out_inner.children.push((other.clone(), other_read.data.clone()));
        }
        out
    }
}
...
impl Tensor {
    ...
    fn uniform(shape: Vec<usize>, low: f32, high: f32, seed: Option<u64>) -> PyResult<Tensor> {
        Ok(Tensor(Tensor_::uniform(shape, low, high, seed)))
    }
    #[staticmethod] 
    fn kaiming_uniform(shape: Vec<usize>, a: f32, seed: Option<u64>) -> PyResult<Tensor> {
        Ok(Tensor(Tensor_::kaiming_uniform(shape, a, seed)))
    }    
    ...
    fn relu(&self) -> PyResult<Tensor> { Ok(Tensor(self.0.relu())) }
